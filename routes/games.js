...
router.post(['/eventLog'], async (req,res,next)=>{
 ...
 router.get(['/newGame'], async (req, res, next) => {
    try{
        ...
        .query(`SELECT l.leagueId, ls.seasonId,  l.name as leagueName, l.gender, l.color as leagueColor, l.shortName as leagueShortName, l.sport, l.dayOfWeek, l.giftCards
...
router.get('/site/league/:leagueId', async (req,res, next)=>{
    try{
        if (req.isAuthenticated()) {
            // console.log(req.user)
        }
...
router.get('/site/season/:seasonId', async (req,res, next)=>{
    try{
        if (req.isAuthenticated()) {
            // console.log(req.user)
        }
...
router.get('/', async (req,res, next)=>{
    try{
        ...
        // const result = await request.query(`Select * from gamesList() where convert(date,DATEADD(s, startunixtime/1000, '19700101')AT TIME ZONE 'UTC' AT TIME ZONE 'Eastern Standard Time') = CONVERT(date,getdate()) order by startUnixTime, location `)
        let sqlStr
        if(req.user?.roles.some(role => ['admin', 'Manager'].includes(role.name))){
             sqlStr = `Select * 
                from gamesList(0) 
                order by startUnixTime, location `
        }else{
